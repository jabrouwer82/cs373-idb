...
def firstSentence(string):
  ...
  # Some ammount of avything followed by a break tag. (First paragraph)
  res1 = res1.group(0)[:-6] if res1 else ''
  res2 = re.search('[^.]+\.', string)
  # Some ammount of not periods followde by a period. (First sentence)
  ...
  # and thus no break tag, and some names have a '.' in them (Robert Downey Jr.)
  # thus truncating early. This picks the longest option as the most correct one
  if len(res) > 138:
    res = res[:138]
    res += '...'
  return res
